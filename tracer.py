...
class Repository:
    ...
    def _create_stub_native_handler(self, target: TraceTarget, decorate: bool) -> str:
        if target.flavor == "objc":
            ...
            if is_objc == 0:
                first_arg = ""
            log_str = "`" + re.sub(r":", objc_arg, target.display_name) + " start " + first_arg + "`"
            if log_str.endswith("} ]`"):
...
if (args[%d] !== null) {
    ...
    \tvar b2 = %d == 1
    \tvar invalidaAddr = b1.charAt(2) === "2"
...

{
  ...
  onLeave(log, retval, state) {
    if (retval !== undefined) {
        log(`%(display_name)s res is ${retval}`);
        ...
        if (b1.length >= 11 && b1.length <= 13 && b2 && invalidaAddr == false) {
          ...
          log(`%(display_name)s res is ${res}`);
        }
    }
    log(`%(display_name)s end`)
  }
}
""" % {
